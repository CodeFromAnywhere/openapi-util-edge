...
const cap = (string: string | undefined, limit: number) => {
  if (!string) {
    return;
  }
 ...
 */
export const summarizeOpenapi = async (
  ...
  const tags = (
    ...
    .concat(
      uniqueOperationTags.map((x) => ({ name: x, description: undefined })),
    )
  ...
  const llmString = taggedOperations
    .map(
      (item) =>
        `# ${item[0].tagName}${item[0].tagDescription ? "\n\n" + item[0].tagDescription : ""}\n\n${item
          .map((item) => {
