export const proxy = (url, method, headers) => async (request) => {
    console.log("entering write endpoint");
    if (method === "post") {
        const body = await request.text();
        console.log({ body });
        // get all headers to pass on
        const headersObject = Array.from(request.headers.keys()).reduce((previous, current) => ({
            ...previous,
        ...
        const responseHeadersObject = Array.from(response.headers.keys()).reduce((previous, current) => ({
            ...previous,
